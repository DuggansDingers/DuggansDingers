...
def _find_schedule_game(meta: dict[str, Any], schedule: list[dict[str, Any]]) -> dict[str, Any] | None:
    # Team IDs are the most reliable bridge between Ballpark Pal and MLB.
    try:
    ...
    if home_id and away_id:
        for game in schedule:
            if int(game.get("home_team_id") or 0) == home_id and int(game.get("away_team_id") or 0) == away_id:
                return game
        for game in schedule:
            if {int(game.get("home_team_id") or 0), int(game.get("away_team_id") or 0)} == {home_id, away_id}:
                return game
    ...
    for game in schedule:
        if game["home"] == home and game["away"] == away:
            return game
    for game in schedule:
        if {game["home"], game["away"]} == {home, away} and home and away:
            return game
    return None
...
def enrich_probable_pitchers(
    ...
    for meta in games_meta:
        ...
        meta["home_team_name"] = match.get("home") or meta.get("home_team_name")
        meta["away_team_name"] = match.get("away") or meta.get("away_team_name")
