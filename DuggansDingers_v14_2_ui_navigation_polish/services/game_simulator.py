...
def simulate_game(
    ...
    for team, team_id in ((away, away_id), (home, home_id)):
        ...
        projected_hits = max(4.5, _num(stats.get("hits_pg"), 8.2) * (0.92 + (run_factor - 1) * 0.25))
        projected_runs = max(
            1.8,
...
def build_game_sims(board: dict[str, Any], iterations: int = 5000) -> list[dict[str, Any]]:
    ...
    games: list[tuple[str, str, str, int, int, list[dict[str, Any]]]] = []
    for key, players in by_game.items():
        ...
        games.append((key, away, home, away_id, home_id, players))

    season = int(str(board.get("date") or "2026")[:4])
    stats = fetch_team_stats(tuple(sorted(team_ids)), season)
    sims = [
        simulate_game(key, away, home, away_id, home_id, players, stats, str(board.get("date")), iterations)
        for key, away, home, away_id, home_id, players in games
    ]
    sims.sort(
