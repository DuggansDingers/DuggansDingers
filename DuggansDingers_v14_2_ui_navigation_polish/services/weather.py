...
def enrich_board_weather(board: dict[str, Any]) -> dict[str, Any]:
    ...
    meta_by_game = {str(meta.get("game_id")): meta for meta in game_meta if meta.get("game_id") is not None}
    ...
    for meta in game_meta:
        ...
        if not home_team:
            try:
                home_team = MLB_TEAM_ID_TO_ABBR.get(int(meta.get("home_team_id") or 0))
            except (TypeError, ValueError):
                home_team = None
        if not home_team:
            try:
                ...
                home_id = 0
            home_player = next(
                (player for player in players_by_game.get(key, []) if int(player.get("team_id") or 0) == home_id),
                None,
            ) if home_id else None
            if home_player:
                home_team = home_player.get("team_name")
        if home_team:
            meta["home_team_name"] = normalize_team_key(str(home_team))
        stadium = find_stadium(home_team, meta.get("venue_name"))
        if not key or not stadium:
            if key:
                detail = f"Stadium location not found for home team={home_team!r}, venue={meta.get('venue_name')!r}"
                results[key] = {"weather_available": False, "weather_error": detail}
            continue
        local_time = _parse_game_time(meta.get("game_time"), stadium["timezone"], slate_date)
        contexts[key] = (stadium, local_time)

    # Unique games are fetched concurrently and then cached for 45 minutes.
    ...
    for player in rankings:
        ...
        elif key in meta_by_game and meta_by_game[key].get("weather_available"):
            player.update({k: v for k, v in meta_by_game[key].items() if k.startswith("weather_") or k in {
                "temperature_f", "apparent_temperature_f", "humidity_pct", "dew_point_f", "precip_probability",
                "precipitation_in", "cloud_cover_pct", "visibility_miles", "surface_pressure_hpa", "wind_speed_mph",
                "wind_gust_mph", "wind_direction_deg", "wind_field_effect", "roof_type", "roof_status", "stadium_name",
                "stadium_timezone",
            }})
