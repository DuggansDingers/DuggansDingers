...
def enrich_board_weather(board: dict[str, Any]) -> dict[str, Any]:
    ...
    # Use the MLB schedule when available, but keep a complete Ballpark Pal
    # fallback. Ballpark Pal's /games payload uses teamHomeId/teamAwayId; older
    # builds missed those fields and every stadium resolved to None.
    ...
    for game in source_games:
        ...
        if not home_team:
            try:
                home_team = MLB_TEAM_ID_TO_ABBR.get(int(game.get("home_team_id") or game.get("teamHomeId") or 0), "")
            except (TypeError, ValueError):
                home_team = ""

        # Recover the home club from enriched hitters when the game record is
        # incomplete. Player rows inherit authoritative schedule fields when
        # the schedule endpoint is available.
        if not home_team:
            sample_players = players_by_game.get(str(game.get("game_id") or key), [])
            for player in sample_players:
                candidate = normalize_team_key(player.get("home_team_name"))
                if candidate:
                    home_team = candidate
                    break
                try:
                    candidate = MLB_TEAM_ID_TO_ABBR.get(int(player.get("home_team_id") or 0), "")
                except (TypeError, ValueError):
                    candidate = ""
                if candidate:
                    home_team = candidate
        ...
        if not stadium:
            results[key] = {
                "weather_available": False,
                "weather_error": (
                    "Stadium location not found after MLB schedule, Ballpark Pal "
                    f"team-id, and player fallback (home={home_team!r}, venue={game.get('venue_name')!r})"
                ),
            }
            continue
    ...
    for player in rankings:
        ...
        if key in results:
            ...
            base_probability = float(player.get("probability") or 0)
            if base_probability > 1:
                base_probability /= 100.0
            weather_impact_value = float(player.get("weather_impact") or 0)
            player["weather_adjusted_probability"] = round(
                max(0.0, min(0.99, base_probability * (1 + weather_impact_value / 100.0))),
                4,
            )
            player["weather_adjusted_dinger_score"] = round(
                max(0.0, min(100.0, float(player.get("dinger_score") or 0) + weather_impact_value * 0.55)),
                1,
            )
            player["weather_card_text"] = (
                f'{player.get("weather_grade") or "—"} • {weather_impact_value:+.1f} HR impact'
                if player.get("weather_available")
                else "Weather pending"
            )
