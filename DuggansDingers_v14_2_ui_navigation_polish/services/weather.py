...
WEATHERAPI_KEY = _secret_value("WEATHERAPI_KEY")
VISUAL_CROSSING_API_KEY = _secret_value("VISUAL_CROSSING_API_KEY")
...
def _visual_crossing(stadium: dict[str, Any], game_time: datetime) -> dict[str, Any]:
    ...
    response = requests.get(
        url,
        params={"unitGroup": "us", "include": "hours", "key": VISUAL_CROSSING_API_KEY, "contentType": "json"},
        headers={"User-Agent": USER_AGENT},
        timeout=25,
    )
    if response.status_code == 429:
...
def enrich_board_weather(board: dict[str, Any]) -> dict[str, Any]:
    ...
    for meta in game_meta:
        ...
        stadium = find_stadium(meta.get("home_team_name"), meta.get("venue_name"))
        if not key or not stadium:
            if key:
                results[key] = {"weather_available": False, "weather_error": "Stadium location not found"}
            continue
