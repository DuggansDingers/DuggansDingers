...
def enrich_players(players: list[dict[str, Any]], target_date: str, season: int) -> tuple[list[dict[str, Any]], str]:
    ...
    if not isinstance(cached, dict):
        # Statcast leaderboards move slowly. Reuse the newest successful
        # snapshot from the same season so a new slate date does not trigger a
        # fresh pybaseball download during every cold start.
        candidates = [
            (key, value) for key, value in cache.items()
            if str(key).startswith("statcast_v2_") and str(key).endswith(f"_{season}")
            and isinstance(value, dict)
            and not str(value.get("status") or "").lower().startswith("unavailable")
            and str(value.get("status") or "").lower() not in {"pybaseball not installed", "pybaseball import failed"}
        ]
        if candidates:
