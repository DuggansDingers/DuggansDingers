...
def _recent_game_form(_: object, row: dict, games: int, color: str) -> str:
    ...
    for value, game_date in zip(series, dates):
        ...
        title = f"{str(game_date or '')[-5:] or 'No date'}: {shown or '0'} HR"
        cells.append(f'<i class="{cls}" title="{esc(title)}"><span>{shown}</span></i>')
    return f'<div class="dd21-game-form compact" style="--form:{color}">' + "".join(cells) + "</div>"
...
<section class="dd-template-hero compact">
  ...
  <aside><div><span>PLAYERS RANKED</span><b>{len(rankings)}</b></div><div><span>TEAMS</span><b>{len(board.get('teams', []))}</b></div><div><span>GAMES</span><b>{safe_int(board.get('games'))}</b></div></aside>
    ...
    columns = [
        ...
        Column("Last 7", "LAST 7", formatter=lambda value, row: _recent_game_form(value, row, 7, "#ff6a42"), width="1.0fr"),
        Column("Last 10", "LAST 10", formatter=lambda value, row: _recent_game_form(value, row, 10, "#b95cff"), width="1.15fr"),
