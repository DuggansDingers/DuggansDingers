...
def render(board: dict) -> None:
    ...
    query_player = str(st.query_params.get("player") or "")
    if query_player.isdigit() and int(query_player) in players_by_id:
        default_id = int(query_player)
    if default_id not in players_by_id:
        default_id = next(iter(players_by_id))

    st.markdown('<div class="dd-player-picker-title"><b>PLAYER INTELLIGENCE CENTER</b><span>Search or choose one of the five highlighted hitters</span></div>', unsafe_allow_html=True)
    search_col, favorite_column = st.columns([4, 1])
    with search_col:
        player_query = st.text_input("Find a player", placeholder="Start typing a player name...", key="player_profile_search")
    candidate_players = list(players_by_id.values())
    if player_query.strip():
        candidate_players = [p for p in candidate_players if player_query.lower() in str(p.get("player_name") or "").lower()]
    candidate_players.sort(key=lambda p: safe_float(p.get("dinger_score")), reverse=True)
    candidates = candidate_players[:5] or [players_by_id[default_id]]

    selected = int(default_id)
    picker_columns = st.columns(5)
    for column, candidate in zip(picker_columns, candidates):
        candidate_id = int(candidate.get("player_id"))
        with column:
            st.markdown(
                f'<div class="dd-player-pick-tile"><img src="https://img.mlbstatic.com/mlb-photos/image/upload/w_180,q_auto:best/v1/people/{candidate_id}/headshot/67/current"><b>{candidate.get("player_name")}</b><span>{candidate.get("team_name")} • {safe_float(candidate.get("dinger_score")):.1f} SCORE</span></div>',
                unsafe_allow_html=True,
            )
            if st.button("OPEN", key=f"pick_profile_{candidate_id}", use_container_width=True):
                st.session_state.selected_player_id = candidate_id
                st.query_params["player"] = str(candidate_id)
                st.rerun()

    query_player = str(st.query_params.get("player") or "")
    if query_player.isdigit() and int(query_player) in players_by_id:
        selected = int(query_player)
    else:
        selected = int(st.session_state.get("selected_player_id") or default_id)
    ...
    c1, c2 = st.columns(2)
    with c1:
        neon_bar_chart(rolling[["Home Runs"]], height=290, palette=["#a65cff"], value_title="Home Runs")
    with c2:
        neon_line_chart(rolling[["HR Rate %"]], height=290, palette=["#20c5ff"], value_title="HR Rate %")
    ...
    if recent_series:
        ...
        neon_bar_chart(pulse, height=240, palette=["#35f083"], value_title="Home Runs")
