...
<article class="dd19-pick dd21-pick" style="--accent:{accent}">
...
def _game_form_cell(_: Any, row: dict[str, Any], games: int, color: str) -> str:
    series = list(row.get("Recent HR Series") or [])[-games:]
    dates = list(row.get("Recent Dates") or [])[-games:]
    if len(series) < games:
        pad = games - len(series)
        series = [None] * pad + series
        dates = [""] * pad + dates
    cells: list[str] = []
    for value, game_date in zip(series, dates):
        if value is None:
            cls, shown = "dnp", "—"
        else:
            count = safe_int(value)
            cls = "multi" if count >= 2 else "hit" if count == 1 else "miss"
            shown = str(count) if count else ""
        label = str(game_date or "")[-5:]
        title = f"{label or 'No date'}: {shown or '0'} HR"
        cells.append(f'<i class="{cls}" title="{esc(title)}"><span>{shown}</span></i>')
    return f'<div class="dd21-game-form" style="--form:{color}">' + "".join(cells) + "</div>"
        ...
        rows.append(
            {
                ...
                "Last 10": safe_int(player.get("last_10_home_runs") or sum(safe_int(v) for v in list(player.get("recent_game_hr_series") or [])[-10:])),
                "Last 30": safe_int(player.get("last_30_home_runs")),
                "Recent HR Series": list(player.get("recent_game_hr_series") or []),
                "Recent Dates": list(player.get("recent_game_dates") or []),
...
def render(board: dict[str, Any]) -> None:
    ...
    columns = [
        ...
        Column("Last 7", "LAST 7", formatter=lambda value, row: _game_form_cell(value, row, 7, "#ff6a42"), width=".96fr"),
        Column("Last 10", "LAST 10", formatter=lambda value, row: _game_form_cell(value, row, 10, "#b95cff"), width="1.12fr"),
