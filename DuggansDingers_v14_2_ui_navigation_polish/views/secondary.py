...
def _sim_narrative(sim: dict) -> str:
    ...
    favorite = away if safe_float(away_metrics.get("win_probability")) >= safe_float(home_metrics.get("win_probability")) else home
    favorite_metrics = sim["teams"][favorite]
    total_hr = safe_float(away_metrics.get("projected_hr")) + safe_float(home_metrics.get("projected_hr"))
    total_k = safe_float(away_metrics.get("projected_so")) + safe_float(home_metrics.get("projected_so"))
    if total_hr >= 3.0:
        scoring_read = "The simulation expects a power-heavy game with multiple run-scoring swings coming through the long ball."
    elif total_k >= 18:
        scoring_read = "The simulation leans toward a pitcher-driven game in which strikeouts suppress extended rallies."
    else:
        scoring_read = "The simulation projects a balanced game with scoring built through a mix of contact, baserunners, and selective power."
    close = abs(safe_float(away_metrics.get("win_probability")) - safe_float(home_metrics.get("win_probability"))) < 0.10
    if close:
        outcome = f"{favorite} owns only a narrow edge, so one bullpen mistake or one extra-base hit could decide the final innings."
    else:
        outcome = f"{favorite} controls the stronger run and win-probability profile, with a projected {safe_float(favorite_metrics.get('win_probability'))*100:.0f}% chance to finish ahead."
    return f"{scoring_read} {outcome}"
...
def _v17_sim_card(sim: dict, active: bool = False) -> str:
    ...
    total = sim.get("sportsbook_total") or sim.get("total_line") or "—"
    over = sim.get("over_odds")
    under = sim.get("under_odds")
    return f'''
<a class="dd-sim-row-link{' active' if active else ''}" href="?view=game-sims&game={quote(key)}" target="_self">
  <div class="dd-sim-row"><div class="match"><img src="{team_logo(sim.get('away_id'))}"><b>{esc(away)} @ {esc(home)}</b><img src="{team_logo(sim.get('home_id'))}"></div>
  ...
  <div><span>TOTAL</span><b>{esc(total)}</b></div>
  <div><span>DRAFTKINGS O/U</span><b>{odds(over) if over else 'PENDING'} / {odds(under) if under else 'PENDING'}</b></div><i>›</i></div>
</a>'''
...
def game_sims(board: dict) -> None:
    ...
    away_metrics = selected["teams"][away]
    home_metrics = selected["teams"][home]
    avg_total = sum(safe_float(sim["teams"][sim["away"]].get("projected_runs")) + safe_float(sim["teams"][sim["home"]].get("projected_runs")) for sim in sims) / max(1, len(sims))
...
<div class="dd-v18-sim-summary">
  ...
  {_v18_sim_stat_card('AVG. TOTAL RUNS', f'{avg_total:.1f}', 'Across slate', '#34c7ff')}
...
<section class="dd-v18-sim-feature">
  ...
  <div class="dd-v18-sim-total"><span>OVER / UNDER</span><b>{esc(total_line)}</b><small>OVER {odds(over) if over else 'PENDING'} &nbsp; | &nbsp; UNDER {odds(under) if under else 'PENDING'}</small><em>DRAFTKINGS</em></div>
...
def _v20_weather_card(game: dict, selected: bool = False) -> str:
    ...
    background = (
        f"background-image:linear-gradient(180deg,rgba(1,7,15,.18),rgba(1,7,15,.86)),url('{scene}');"
        if scene else ""
    )
    return f'''
...
def _v20_weather_detail(board: dict, game: dict) -> str:
    ...
    background = (
        f"background-image:linear-gradient(90deg,rgba(1,7,15,.10),rgba(1,7,15,.72)),url('{scene}');"
        if scene else ""
    )
...
def weather_center(board: dict) -> None:
    ...
    providers = ", ".join(summary.get("providers") or ["WeatherAPI / NWS"])
    st.markdown(
        f'''<section class="dd20-weather-hero"><div><span>MLB HOME RUN INTELLIGENCE</span><h1>BALLPARK <em>WEATHER COMMAND</em></h1><p>Three AI-illustrated stadium scenes per row. Open any matchup for its complete game-time weather, field-direction carry, and weather-adjusted hitter board.</p></div><aside><div><span>GAMES TODAY</span><b>{len(games)}</b></div><div><span>CONNECTED</span><b>{summary.get("games_available", 0)}</b></div><div><span>DATA SOURCE</span><b>{esc(providers)}</b></div></aside></section>''',
        unsafe_allow_html=True,
    )
    if not games:
