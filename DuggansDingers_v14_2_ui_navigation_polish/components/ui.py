...
def headshot(player_id: Any, width: int = 300) -> str:
    return f"https://img.mlbstatic.com/mlb-photos/image/upload/w_{width},q_auto:best/v1/people/{player_id}/headshot/silo/current"
...
<div class="dd-feature-card" style="--accent:{accent}">
  ...
  {player_image}<div class="dd-feature-shade"></div>
...
def player_card_html(player: dict[str, Any], rank: int | None = None) -> str:
    ...
    pitcher_detail = (
        f'{player.get("opposing_pitcher_hand") or "—"}HP • {safe_float(player.get("pitcher_hr9")):.2f} HR/9 • {safe_float(player.get("pitcher_era")):.2f} ERA'
        if player.get("pitching_data_available") else "Probable starter pending"
    )
    return f'''
<div class="dd-player-card v16" style="--accent:{accent}">
  <div class="dd-player-card-score">{score:.1f}<small>Dinger Score</small></div>
  <div class="dd-player-card-head"><img src="{headshot(player.get('player_id'),220)}"><div><div class="dd-player-card-name">{esc(player.get('player_name'))}</div><div class="dd-player-card-team"><img src="{team_logo(player.get('team_id'))}">{esc(player.get('team_name'))} • {esc(player.get('position','—'))} • vs {esc(player.get('opponent','—'))}</div></div></div>
  <div class="dd-player-card-pitcher"><span>PROBABLE PITCHER</span><b>{esc(pitcher)}</b><small>{esc(pitcher_detail)}</small></div>
...
<div class="dd-ticket-leg">
  ...
  <div class="dd-ticket-metric" style="color:{weather_color(player)}">{safe_float(player.get('weather_impact')):+.1f}<small>{esc(player.get('weather_grade') or 'Weather')}</small></div>
