...
def get_player_hitting_stats(player_id: Any, season: int, target_date: str, stats_cache: dict[str, Any]) -> dict[str, Any]:
    ...
    # Reuse the newest recent snapshot for this player when available. This
    # makes a cold Streamlit deploy load quickly while the live cache refreshes.
    suffix = f"_{season}_{player_id}"
    stale_candidates = [
        (str(key), value) for key, value in stats_cache.items()
        if str(key).endswith(suffix) and isinstance(value, dict) and "last_30_home_runs" in value
    ]
    if stale_candidates:
        stale_candidates.sort(key=lambda pair: pair[0], reverse=True)
        newest_key, newest = stale_candidates[0]
        try:
            cached_date = newest_key.split("_")[1]
            age_days = (date.fromisoformat(target_date) - date.fromisoformat(cached_date)).days
        except (ValueError, IndexError):
            age_days = 99
        if 0 <= age_days <= 2:
            stats_cache[cache_key] = newest
            return newest
...
def game_metadata(game: dict[str, Any]) -> dict[str, Any]:
    ...
    # Ballpark Pal's live /games feed uses teamHomeId/teamAwayId.
    # Earlier builds only looked for homeTeamId/awayTeamId, which left every
    # stadium and probable-pitcher matchup unresolved.
    home_id = first_value(game.get("teamHomeId"), game.get("homeTeamId"), home.get("id"))
    away_id = first_value(game.get("teamAwayId"), game.get("awayTeamId"), away.get("id"))
    try:
        home_lookup = MLB_TEAM_ID_TO_ABBR.get(int(home_id)) if home_id is not None else None
    except (TypeError, ValueError):
        home_lookup = None
    try:
        away_lookup = MLB_TEAM_ID_TO_ABBR.get(int(away_id)) if away_id is not None else None
    except (TypeError, ValueError):
        away_lookup = None
    return {
        "game_id": first_value(game.get("gameId"), game.get("id"), game.get("gamePk")),
        "game_time": first_value(game.get("gameTimeUTC"), game.get("gameTimeFull"), game.get("gameTime"), game.get("gameDate"), game.get("startTime")),
        "home_team_id": home_id,
        "away_team_id": away_id,
        "home_team_name": first_value(game.get("homeTeamAbbreviation"), home.get("abbreviation"), home_lookup, home.get("name")),
        "away_team_name": first_value(game.get("awayTeamAbbreviation"), away.get("abbreviation"), away_lookup, away.get("name")),
        "venue_id": first_value(game.get("venueId"), venue.get("id")),
...
def get_home_run_rankings(target_date: str | None = None) -> dict[str, Any]:
    ...
    def fetch_game_props(game: dict[str, Any]) -> list[dict[str, Any]]:
        meta = game_metadata(game)
    ...
    with ThreadPoolExecutor(max_workers=min(12, max(1, len(games)))) as executor:
        futures = [executor.submit(fetch_game_props, game) for game in games]
        for future in as_completed(futures):
            try:
                home_run_props.extend(future.result())
            except Exception:
                continue
    ...
    with ThreadPoolExecutor(max_workers=min(20, max(1, len(home_run_props)))) as executor:
    ...
    # Repair incomplete Ballpark Pal game metadata from real MLB team IDs and
    # the enriched player records. Weather and probable-pitcher matching depend
    # on these abbreviations being present.
    players_by_game: dict[str, list[dict[str, Any]]] = {}
    for player in home_run_props:
        players_by_game.setdefault(str(player.get("game_id") or ""), []).append(player)
    for key, meta in game_map.items():
        try:
            home_id = int(meta.get("home_team_id"))
        except (TypeError, ValueError):
            home_id = 0
        try:
            away_id = int(meta.get("away_team_id"))
        except (TypeError, ValueError):
            away_id = 0
        meta["home_team_name"] = meta.get("home_team_name") or MLB_TEAM_ID_TO_ABBR.get(home_id)
        meta["away_team_name"] = meta.get("away_team_name") or MLB_TEAM_ID_TO_ABBR.get(away_id)
        for player in players_by_game.get(key, []):
            try:
                team_id = int(player.get("team_id"))
            except (TypeError, ValueError):
                team_id = 0
            if team_id == home_id and player.get("team_name"):
                meta["home_team_name"] = player.get("team_name")
            elif team_id == away_id and player.get("team_name"):
                meta["away_team_name"] = player.get("team_name")
            player["home_team_name"] = meta.get("home_team_name")
            player["away_team_name"] = meta.get("away_team_name")
