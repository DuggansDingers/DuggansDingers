...
/* ===== V13 EXACT-TEMPLATE REDESIGN OVERRIDES ===== */
...
.block-container{max-width:1540px!important;padding:.65rem 1.1rem 2.2rem!important}
.stApp{background:
 radial-gradient(circle at 100% 8%,rgba(0,102,255,.30),transparent 27%),
 radial-gradient(circle at 0% 45%,rgba(255,55,0,.16),transparent 25%),
 linear-gradient(180deg,#01050d 0%,#031329 52%,#010711 100%)!important}
.dd-brand-header{position:relative;display:grid;grid-template-columns:190px 1fr 210px;align-items:center;gap:18px;min-height:116px;padding:4px 10px 8px;border-bottom:1px solid rgba(28,118,205,.42)}
.dd-header-logo{width:165px;height:105px;object-fit:contain;filter:drop-shadow(0 0 16px rgba(255,90,20,.48))}
.dd-brand-center{text-align:center}
.dd-brand-title{font-family:'Barlow Condensed',sans-serif;font-style:italic;font-size:clamp(2.8rem,5vw,5rem);font-weight:900;line-height:.85;letter-spacing:-.045em;text-transform:uppercase;color:#fff;text-shadow:0 4px 0 #5f6976,0 0 26px rgba(255,255,255,.16)}
.dd-brand-title span{margin-left:.06em}
.dd-brand-rule{display:flex;align-items:center;justify-content:center;gap:12px;margin-top:12px}
.dd-brand-rule i{height:2px;width:110px;background:linear-gradient(90deg,transparent,#168dff)}
.dd-brand-rule i:last-child{background:linear-gradient(90deg,#168dff,transparent)}
.dd-brand-rule b{font-family:'Barlow Condensed',sans-serif;color:#2597ff;letter-spacing:.28em;font-size:.9rem}
.dd-live-pill{justify-self:end;border:1px solid #1f7ece;background:#031326;border-radius:8px;padding:12px 14px;font-family:'Barlow Condensed',sans-serif;font-weight:800;color:#dceeff;box-shadow:inset 0 0 18px rgba(0,129,255,.08)}
.dd-live-pill span{display:inline-block;width:8px;height:8px;border-radius:50%;background:#43f88d;margin-right:8px;box-shadow:0 0 12px #43f88d}
div[data-testid="stRadio"]{background:linear-gradient(180deg,#06162a,#020c18);border:1px solid #144d82;border-radius:8px;padding:6px 8px;overflow-x:auto;white-space:nowrap}
div[data-testid="stRadio"]>div{gap:2px!important;flex-wrap:nowrap!important}
div[data-testid="stRadio"] label{background:transparent!important;border:0!important;border-radius:6px!important;padding:.48rem .72rem!important;min-width:max-content}
div[data-testid="stRadio"] label:has(input:checked){background:linear-gradient(180deg,#0f78ee,#064daa)!important;box-shadow:0 0 16px rgba(28,143,255,.45)}
div[data-testid="stRadio"] label p{font-family:'Barlow Condensed',sans-serif!important;text-transform:uppercase;font-weight:800!important;font-size:.88rem!important;color:#eaf4ff!important}
.dd-status-line{font-family:'Barlow Condensed',sans-serif;text-transform:uppercase;color:#8ea7bf;font-weight:700;letter-spacing:.08em;padding:6px 4px 12px}
.dd-status-line b{margin-left:16px;color:#d9ebff}
.dd-status-dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:#2cff8b;box-shadow:0 0 10px #2cff8b;margin-right:8px}
.dd-feature-grid{grid-template-columns:repeat(6,minmax(0,1fr))!important;gap:12px!important}
.dd-feature-card{min-height:318px!important;border-radius:8px!important;background:linear-gradient(180deg,rgba(7,19,38,.96),rgba(2,7,14,.98))!important;box-shadow:inset 0 0 24px rgba(0,0,0,.45),0 0 20px color-mix(in srgb,var(--accent) 24%,transparent)!important}
.dd-feature-title{font-family:'Barlow Condensed',sans-serif!important;font-style:italic!important;font-size:1.25rem!important;text-transform:uppercase!important;text-align:center!important;background:linear-gradient(180deg,rgba(255,255,255,.04),transparent)!important}
.dd-feature-player{height:152px!important;width:auto!important;max-width:95%!important;left:50%!important;transform:translateX(-50%)!important;object-fit:contain!important;object-position:center bottom!important;filter:drop-shadow(0 12px 18px rgba(0,0,0,.75))!important}
.dd-feature-brand{display:none!important}
.dd-feature-content{padding:176px 12px 12px!important;text-align:center!important}
.dd-feature-name{font-size:1.03rem!important}
.dd-feature-meta{font-size:.63rem!important;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.dd-feature-value{font-size:2rem!important}
.dd-feature-stats{display:none!important}
.dd-table-shell{border-color:#1b619d!important;border-radius:8px!important}
.dd-rank-table thead{background:#06162a!important}
.dd-rank-table th{font-family:'Barlow Condensed',sans-serif!important;text-transform:uppercase!important;color:#dfeeff!important}
.dd-rank-table tr:hover td{background:rgba(19,104,185,.10)!important}
.dd-bottom-strip{border-radius:8px!important;background:linear-gradient(180deg,#041329,#020912)!important}
.dd-weather-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;margin:14px 0 24px}
.dd-weather-card{position:relative;overflow:hidden;border:1px solid color-mix(in srgb,var(--weather) 70%,#0a2440);border-radius:12px;background:linear-gradient(180deg,#071a30,#020a14);box-shadow:0 0 26px color-mix(in srgb,var(--weather) 15%,transparent)}
.dd-weather-card-head{display:flex;justify-content:space-between;align-items:center;padding:15px 17px;border-bottom:1px solid rgba(88,159,224,.22)}
.dd-weather-card-head span{font-family:'Barlow Condensed',sans-serif;color:var(--weather);font-weight:900;letter-spacing:.14em}
.dd-weather-card-head h3{margin:2px 0 0;font-family:'Barlow Condensed',sans-serif;font-size:1.55rem;text-transform:uppercase}
.dd-weather-grade{font-family:'Barlow Condensed',sans-serif;font-size:2.25rem;font-weight:900;color:var(--weather);text-align:center}
.dd-weather-grade small{display:block;font-size:.62rem;color:#91a8bd;letter-spacing:.09em}
.dd-ballpark{position:relative;height:330px;overflow:hidden;background:
 radial-gradient(ellipse at 50% 112%,#986233 0 18%,transparent 18.5%),
 radial-gradient(ellipse at 50% 100%,#18572d 0 57%,#0d3c21 58% 65%,transparent 66%),
 linear-gradient(180deg,#04101f,#061a2b)}
.dd-ballpark:before{content:'';position:absolute;left:7%;right:7%;bottom:21%;height:65%;border:3px solid rgba(255,255,255,.70);border-bottom:0;border-radius:50% 50% 0 0;opacity:.72}
.dd-outfield-arc{position:absolute;left:17%;right:17%;bottom:23%;height:52%;border:2px dashed rgba(255,255,255,.28);border-bottom:0;border-radius:50% 50% 0 0}
.dd-infield{position:absolute;left:50%;bottom:27px;width:126px;height:126px;transform:translateX(-50%) rotate(45deg);background:#a87342;border:3px solid rgba(255,255,255,.72)}
.dd-diamond{position:absolute;left:50%;bottom:55px;width:72px;height:72px;transform:translateX(-50%) rotate(45deg);background:#176132;border:2px solid rgba(255,255,255,.7)}
.dd-homeplate{position:absolute;left:50%;bottom:16px;transform:translateX(-50%);width:18px;height:14px;background:white;clip-path:polygon(0 0,100% 0,85% 70%,50% 100%,15% 70%)}
.dd-zone{position:absolute;text-align:center;color:white;text-shadow:0 0 12px currentColor}
.dd-zone b{display:block;font-family:'Barlow Condensed',sans-serif;font-size:2rem;color:var(--weather)}
.dd-zone small{font-family:'Barlow Condensed',sans-serif;font-weight:900;letter-spacing:.12em}
.dd-zone-lf{left:14%;top:34%}.dd-zone-cf{left:50%;transform:translateX(-50%);top:18%}.dd-zone-rf{right:14%;top:34%}
.dd-wind-arrow{position:absolute;left:50%;top:44%;font-size:3.2rem;color:#5be9ff;text-shadow:0 0 18px #159cff;transform-origin:center}
.dd-weather-metrics{display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid rgba(88,159,224,.22)}
.dd-weather-metrics div{padding:12px 8px;text-align:center;border-right:1px solid rgba(88,159,224,.18)}
.dd-weather-metrics div:last-child{border-right:0}
.dd-weather-metrics b{display:block;font-family:'Barlow Condensed',sans-serif;font-size:1.28rem;color:#fff}
.dd-weather-metrics span{font-size:.61rem;text-transform:uppercase;color:#8fa5b9;font-weight:800}
div[data-testid="stExpander"]{border:1px solid #154d80!important;border-radius:10px!important;background:linear-gradient(180deg,#06172b,#030d19)!important;margin-bottom:12px}
div[data-testid="stExpander"] summary{font-family:'Barlow Condensed',sans-serif!important;font-weight:900!important;text-transform:uppercase!important;color:#fff!important}
@media(max-width:1050px){
 .dd-brand-header{grid-template-columns:115px 1fr}.dd-live-pill{display:none}.dd-header-logo{width:110px;height:78px}.dd-brand-title{font-size:3rem}
 .dd-feature-grid{grid-template-columns:repeat(3,minmax(0,1fr))!important}
 .dd-weather-grid{grid-template-columns:1fr}
}
@media(max-width:700px){
 .block-container{padding:.35rem .55rem 1.5rem!important}
 .dd-brand-header{grid-template-columns:72px 1fr;min-height:78px;gap:8px;padding:0 2px 5px}
 .dd-header-logo{width:68px;height:64px}.dd-brand-title{font-size:2rem;letter-spacing:-.03em}.dd-brand-rule{margin-top:7px}.dd-brand-rule i{width:28px}.dd-brand-rule b{font-size:.52rem;letter-spacing:.15em}
 div[data-testid="stRadio"]{border-radius:7px;padding:4px;margin-top:2px}
 div[data-testid="stRadio"] label{padding:.4rem .58rem!important}
 div[data-testid="stRadio"] label p{font-size:.76rem!important}
 .dd-feature-grid{grid-template-columns:1fr!important;gap:9px!important}
 .dd-feature-card{min-height:132px!important;display:grid!important;grid-template-columns:112px 1fr!important}
 .dd-feature-title{position:absolute!important;left:123px!important;top:10px!important;font-size:.92rem!important;text-align:left!important;background:none!important}
 .dd-feature-player{left:8px!important;transform:none!important;bottom:0!important;height:118px!important;width:103px!important;object-position:center bottom!important}
 .dd-feature-content{padding:35px 10px 8px 123px!important;text-align:left!important}
 .dd-feature-name{font-size:1rem!important}.dd-feature-meta{font-size:.58rem!important}.dd-feature-value{font-size:1.7rem!important}
 .dd-rank-table{min-width:980px!important}.dd-table-shell{overflow-x:auto!important}
 .dd-weather-card-head{padding:11px 12px}.dd-ballpark{height:245px}.dd-weather-metrics{grid-template-columns:repeat(2,1fr)}.dd-weather-metrics div:nth-child(2){border-right:0}
 .dd-bottom-strip{grid-template-columns:1fr 1fr!important}
}
